...
def Time_Gannt(RobotComponents, TimeStamps, Names_RobotComponent):
    ...
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig("特征码定位-闭环验证机器人各部件用时统计.png")
    plt.show()
...
def histogram(data):
    ...
    plt.tight_layout()
    plt.savefig("特征码定位-闭环验证视觉计算时间直方图统计.png")
    plt.show()
...
if __name__ == "__main__":
    lines = ReadFile('Time_char_i5_closeloop_1130.txt')
