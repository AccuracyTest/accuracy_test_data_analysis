...
if __name__ == "__main__":
    line = ReadFile("char_i5_openloop_Euler_1130.txt")
    ...
    plt.title('特征码定位-开环验证位置定位精度测试结果', weight='bold', fontproperties=my_font, fontsize=14)
    plt.grid()
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('特征码定位-开环验证位置定位精度测试结果.png')
    plt.show()

    figure2 = plt.figure(figsize=(15, 4))
    plt.suptitle('特征码定位-开环验证姿态定位精度测试结果', weight='bold', fontproperties=my_font, fontsize=14)
    ...
    ax3.legend(loc='best')
    plt.tight_layout()
    plt.savefig('特征码定位-开环验证姿态定位精度测试结果.png')
    plt.show()
